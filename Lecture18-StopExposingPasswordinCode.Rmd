...
```{r, eval=FALSE, include=TRUE}
# data frame containing data table
...
#### Another option (all in once!)

```{r, eval=FALSE, include=TRUE}
# function that connects to SQL and return DF object, there is no password in clear text...!!!
getDataFromSQL_v2 <- function(wrkTableName = "WRK_TABLE_NAME",
                           connection = "SQL_SERVER_NAME",
                           userID = "SQL_USER_ID",
                           passwEnc = "Public.rds"){
  
  out <- readRDS(passwEnc)

  # decrypting the password using public data list and private key
  pass <- decrypt_envelope(out$data, out$iv, out$session, "~/ShinyApps/.rsa/key.pem", password = "") %>% 
    unserialize()
  
  ch2<-odbcConnect(connection,uid = userID,pwd = pass)
  if(ch2 == -1){
    print("Error: connection was not established")
...
# use example
DF_Data <- getDataFromSQL_v2("DB_NAME.dbo.WRK_TABLENAME")
```


#### Conclusion

We can now encrypt password in our project and do so with plain usage of openssl library. 
I would recommend you to always use this method and avoid using password in clear text in any of your R scripts
