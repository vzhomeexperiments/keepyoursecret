...
Please pause the video and carefully read those situations

Let's start with situations when we can encrypt but could not decrypt it anymore ...

##### Encrypting with Public Key, deleting Information, not having corresponding Private Key...

This is an example how can it happen that we can generate new private key and accidentally overwrite already existing one. Please take a moment and possibly execute this chunc of code below

```{r, eval=FALSE, include=TRUE}
library(openssl)
library(tidyverse)

# 1. in this situation I have my public key in the R Project Folder
file.exists("public.key")

# 2. I would then create and encrypt my text using that public key
...
# 3. and delete information...
rm(mySECRET)

# 4. This time I would SIMULATE disaster... I am creating new pairs of keys...
# generating and writing new private key
rsa_keygen(bits = 2099) %>% 
  write_pem(path = "private.key", password = "udemy")

# 5. Now I will attempt to decrypt my encrypted information...
...
Probably among different strategies the best one is the one that allows automation... for example, I would check presence of my corresponding private key before I can encrypt
...
##### Examples of passphrase / private key leaks:
...
* If you think that you submitted some private key to the repository - immediately decrypt your information and replace your private keys, delete old 'commits'
* Use 'pipe' operator to avoid storing extra objects in R Environment
* Clean History, Console and R Environment
* Delete .Rhistory file
...
```
 ...
 Just a side note!!! As you may see managing your secrets is really difficult today!!!
...
Before that I will highly recommend you to visit the last Assignment and QUIZ where I put a lot of questions designed to keep yourself alerted on the risks...